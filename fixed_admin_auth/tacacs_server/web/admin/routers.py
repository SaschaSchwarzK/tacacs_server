...
from ..web import (
    ...
    get_config,
    get_device_service,
    get_local_user_group_service,
    get_local_user_service,
    get_radius_server,
    get_tacacs_server,
    logger,
)
from .auth_helpers import check_admin_auth
...
def get_device_service_dependency() -> DeviceService:
...
def get_user_service_dependency() -> LocalUserService:
...
def get_user_group_service_dependency() -> LocalUserGroupService:
...
async def admin_guard(request: Request) -> None:
    ...
    Correct: only inspect cookies/headers without consuming the body.
    """
    # ✅ Only check cookies/headers (do not read body)
...
@admin_router.get("/webhooks", response_class=HTMLResponse, name="admin_webhooks")
...
@admin_router.get("", response_class=HTMLResponse, name="admin_home")
@admin_router.get("/", response_class=HTMLResponse, name="admin_home_slash")
async def admin_home(request: Request, _: None = Depends(admin_guard)):
    device_service = get_device_service()
    user_service = get_local_user_service()
    user_group_service = get_local_user_group_service()
...
async def list_devices(
    request: Request,
    service: DeviceService = Depends(get_device_service_dependency),
    _: None = Depends(admin_guard),
...
async def create_device(
    ...
    # Check auth first (doesn't consume body)
    check_admin_auth(request)
...
async def list_groups(
    request: Request,
    service: DeviceService = Depends(get_device_service_dependency),
    user_group_service: LocalUserGroupService = Depends(
        get_user_group_service_dependency
    ),
    _: None = Depends(admin_guard),
):
    data = [
...
async def list_user_groups(
    request: Request,
    service: LocalUserGroupService = Depends(get_user_group_service_dependency),
    _: None = Depends(admin_guard),
):
...
async def list_users(
    request: Request,
    service: LocalUserService = Depends(get_user_service_dependency),
    group_service: LocalUserGroupService = Depends(get_user_group_service_dependency),
    _: None = Depends(admin_guard),
    search: str | None = None,
...
async def create_user(
    ...
    # Check auth before parsing JSON body
    check_admin_auth(request)
    try:
        # Parse and validate JSON payload
        try:
