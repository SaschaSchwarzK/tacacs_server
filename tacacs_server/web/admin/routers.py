...
from ..monitoring import logger  # reuse monitoring logger for now
...
def get_user_group_service() -> LocalUserGroupService:
    ...
    if not service:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="User group service unavailable")
    return service


def _parse_allowed_groups(value) -> Optional[list[str]]:
    if value is None:
        return None
    if isinstance(value, str):
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="allowed_user_groups must be a list or comma/newline separated string") from exc
    tokens = []
...
def _parse_int(value, field: str, *, required: bool = False) -> Optional[int]:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"{field} must be an integer") from exc


_SENSITIVE_KEYWORDS = ("secret", "password", "token", "key", "hash")


def _calc_metric(stats: dict, *, total_key: str, success_key: str, failure_key: str) -> dict:
...
def _sanitize_config_data(value: Any, *, sensitive: bool = False) -> Any:
    if isinstance(value, dict):
        ...
        for key, inner in value.items():
            result[key] = _sanitize_config_data(inner, sensitive=sensitive or _is_sensitive_key(key))
        return result
...
async def admin_home(request: Request, _: None = Depends(admin_guard)):
    ...
    summary = {
        ...
        "user_groups": len(user_group_service.list_groups()) if user_group_service else 0,
    }

    raw_stats = tacacs_server.get_stats() if tacacs_server else {}
    health = tacacs_server.get_health_status() if tacacs_server else {}

    auth_summary = _calc_metric(raw_stats, total_key="auth_requests", success_key="auth_success", failure_key="auth_failures")
    author_summary = _calc_metric(raw_stats, total_key="author_requests", success_key="author_success", failure_key="author_failures")
    acct_summary = _calc_metric(raw_stats, total_key="acct_requests", success_key="acct_success", failure_key="acct_failures")
    ...
    if radius_server:
        ...
        radius_acct = _calc_metric(
            {
                ...
                'auth_failures': r_stats.get('acct_requests', 0) - r_stats.get('acct_responses', 0),
            },
        ...
        radius_summary = {
            "clients": len(clients),
            "auth": _calc_metric(r_stats, total_key="auth_requests", success_key="auth_accepts", failure_key="auth_rejects"),
            "acct": radius_acct,
        }
    ...
    if device_service:
        for record in device_service.list_devices()[:5]:
            ...
            device_samples.append({
                ...
                "has_tacacs_secret": bool(getattr(group, "tacacs_secret", None)) if group else False,
                "has_radius_secret": bool(getattr(group, "radius_secret", None)) if group else False,
            })
    ...
    config_source = None
    if config:
        config_source = getattr(config, "config_source", None) or getattr(config, "config_file", "config/tacacs.conf")
    ...
    system_summary = {
        ...
        "memory_human": f"{_format_bytes(mem_used)} / { _format_bytes(mem_total)}" if mem_total is not None else 'N/A',
...
async def view_config(
    ...
    source = getattr(config, "config_source", getattr(config, "config_file", "config/tacacs.conf"))
...
async def view_config_trailing_slash():
    return RedirectResponse(url="/admin/config", status_code=status.HTTP_307_TEMPORARY_REDIRECT)
    ...
    data = [
        {
            ...
            "group_radius_secret": bool(record.group.radius_secret) if record.group else False,
            "group_tacacs_secret": bool(record.group.tacacs_secret) if record.group else False,
        }
...
async def create_device(
    ...
    try:
        ...
        logger.info("Admin UI: created device id=%s name=%s", record.id, payload.get("name"))
        return {"id": record.id}
...
async def list_groups(
    ...
    if "application/json" in accept or request.query_params.get("format") == "json":
        return data
    user_group_options = sorted(record.name for record in user_group_service.list_groups())
    return templates.TemplateResponse(
        "admin/groups.html",
        {
            "request": request,
...
async def create_group(
    ...
    try:
        group = service.create_group(
            ...
            allowed_user_groups=_parse_allowed_groups(payload.get("allowed_user_groups")),
        )
        logger.info("Admin UI: created device group id=%s name=%s", group.id, payload.get("name"))
        return {"id": group.id}
...
async def update_group(
    ...
    try:
        group = service.update_group(
            ...
            radius_profile=payload.get("radius_profile"),
            metadata=payload.get("metadata"),
            radius_secret=payload.get("radius_secret") if "radius_secret" in payload else UNSET,
            tacacs_secret=payload.get("tacacs_secret") if "tacacs_secret" in payload else UNSET,
            device_config=payload.get("device_config") if "device_config" in payload else UNSET,
            allowed_user_groups=_parse_allowed_groups(payload.get("allowed_user_groups")) if "allowed_user_groups" in payload else UNSET,
        )
...
async def delete_group(
    ...
    try:
        ...
        logger.info("Admin UI: deleted device group id=%s cascade=%s", group_id, cascade)
    except GroupNotFound as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except DeviceValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
...
async def create_user_group(
    ...
    try:
        record = service.create_group(
            ...
            privilege_level=_parse_int(payload.get("privilege_level"), "privilege_level") or 1,
        )
...
async def update_user_group(
    ...
    try:
        record = service.update_group(
            ...
            privilege_level=_parse_int(payload.get("privilege_level"), "privilege_level") if "privilege_level" in payload else None,
        )
