...
class AAAHandlers:
    ...
    def cleanup_session(self, session_id: int) -> None:
        ...
        stale_keys = [key for key in list(self.auth_sessions) if key.startswith(f'{session_id}_')]
    ...
    def handle_authentication(self, packet: TacacsPacket, device: Optional[Any] = None) -> TacacsPacket:
        ...
        try:
            if len(packet.body) < 8:
                ...
                response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_ERROR)
            ...
            logger.debug('TACACS auth request: user=%s, type=%s, action=%s, seq=%s', safe_user, authen_type, action, packet.seq_no)
            self._remember_username(packet.session_id, user)
            if packet.seq_no == 1:
                return self._handle_auth_start(packet, action, authen_type, user, port, rem_addr, data, priv_lvl, device)
            else:
                return self._handle_auth_continue(packet, user, data)
        except Exception as e:
            logger.error(f'Authentication error: {e}')
            response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_ERROR, 'Internal server error')
            self.cleanup_session(packet.session_id)
            return response
        ...
        try:
            ...
            for arg_len in arg_lengths:
                ...
                if '=' in arg_str:
                    ...
                    args[arg_str] = ''
                offset += arg_len
            logger.debug('TACACS accounting request: user=%s, flags=%s, args=%s', self._safe_user(user), flags, args)
            return self._process_accounting(packet, user, port, rem_addr, flags, authen_service, priv_lvl, args, device)
        except Exception as e:
    ...
    def _handle_auth_start(
        ...
        if authen_type == TAC_PLUS_AUTHEN_TYPE.TAC_PLUS_AUTHEN_TYPE_PAP:
            ...
            if authenticated:
                ...
                response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_FAIL, 'Authentication failed')
                self.cleanup_session(packet.session_id)
                return response
            ...
            self._log_auth_result(packet.session_id, user, device, False, 'CHAP authentication not implemented')
            response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_ERROR, 'CHAP authentication not implemented')
            self.cleanup_session(packet.session_id)
            return response
        else:
            self._log_auth_result(packet.session_id, user, device, False, f'Unsupported authentication type {authen_type}')
            response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_ERROR, f'Unsupported authentication type: {authen_type}')
            self.cleanup_session(packet.session_id)
            return response
    ...
    def _handle_auth_continue(self, packet: TacacsPacket, user: str, data: bytes) -> TacacsPacket:
        ...
        if session_info['step'] == 'username':
            ...
            if authenticated:
                ...
                device = self.session_device.get(packet.session_id)
                self._log_auth_result(packet.session_id, username, device, False, detail)
                response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_FAIL, 'Authentication failed')
                self.cleanup_session(packet.session_id)
                return response
        else:
            response = self._create_auth_response(packet, TAC_PLUS_AUTHEN_STATUS.TAC_PLUS_AUTHEN_STATUS_ERROR, 'Invalid authentication step')
            self.cleanup_session(packet.session_id)
            return response
    ...
    def _process_accounting(
        ...
        if self.db_logger.log_accounting(record):
            response = self._create_acct_response(packet, TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_SUCCESS, 'Accounting record logged successfully')
        else:
            response = self._create_acct_response(packet, TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_ERROR, 'Failed to log accounting record')
        self.cleanup_session(packet.session_id)
        return response
    ...
    def _authenticate_user(self, username: str, password: str, client_ip: str = None) -> tuple[bool, str]:
        ...
        for backend in self.auth_backends:
            try:
                ...
                logger.warning("Auth error with %s for %s: %s", backend.name, username, exc)
            except Exception as exc:
                last_error = f'backend={backend.name} error={exc}'
                logger.error("Unexpected authentication error with %s: %s", backend.name, exc)
