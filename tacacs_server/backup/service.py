...
from .archive_utils import create_tarball as create_tar, extract_tarball as extract_tar
...
class BackupService:
    def __init__(self, config: TacacsConfig, execution_store: BackupExecutionStore):
        ...
        self.temp_dir = Path("data/backup_temp")
        self._ensure_temp_dir()
        self.instance_name = (
            self.config.config_store.get_instance_name() if self.config.config_store else "tacacs-server"
        )
        self.instance_id = (
            self.config.config_store.get_metadata("instance_id") if self.config.config_store else None
        )
    ...
    def _create_manifest(self, backup_dir: str, backup_type: str, triggered_by: str) -> dict:
        ...
        manifest: dict[str, Any] = {
            ...
            "config_info": {
                ...
                "has_overrides": len(getattr(self.config, "overridden_keys", {}) or {}) > 0,
            },
            "contents": [],
            "total_size_bytes": 0,
        }
    ...
    def execute_backup(self, destination_id: str, triggered_by: str, backup_type: str = "manual") -> str:
        ...
        try:
            ...
            _logger.info("backup_start", execution_id=execution_id, destination_id=destination_id)
            ...
            databases_to_backup = [
                ...
                ("tacacs_accounting.db", self.config.get_database_config()["accounting_db"]),
                ("metrics_history.db", self.config.get_database_config()["metrics_history_db"]),
                ("audit_trail.db", self.config.get_database_config()["audit_trail_db"]),
            ]
            for backup_name, source_path in databases_to_backup:
                try:
                    if source_path and os.path.exists(source_path):
                        self._export_database(source_path, os.path.join(backup_dir, backup_name))
                except Exception as exc:
                    _logger.warning("backup_db_export_failed", db=backup_name, error=str(exc))

            # Step 4: Export configuration
            try:
                if not self.config.is_url_config() and getattr(self.config, "config_file", None):
                    if os.path.exists(self.config.config_file):
                        shutil.copy2(self.config.config_file, os.path.join(backup_dir, "tacacs.conf"))
            except Exception as exc:
                _logger.warning("backup_config_copy_failed", error=str(exc))

            config_dict = self.config._export_full_config()
            with open(os.path.join(backup_dir, "config_export.json"), "w", encoding="utf-8") as f:
            ...
            destination = create_destination(dest_config["type"], json.loads(dest_config["config_json"]))
            ...
            _logger.info("backup_completed", execution_id=execution_id, archive_size=archive_size)
            return execution_id
        except Exception as exc:
            _logger.exception("backup_failed", execution_id=execution_id)
            try:
                self.execution_store.update_execution(
            ...
            try:
                ...
                if dest_config:
                    destination = create_destination(dest_config["type"], json.loads(dest_config["config_json"]))
                    destination.apply_retention_policy(int(dest_config.get("retention_days", 30)))
            except Exception:
                pass

    def restore_backup(self, source_path: str, destination_id: str | None = None, components: list[str] | None = None) -> tuple[bool, str]:
        """
        ...
        try:
            ...
            if destination_id:
                ...
                destination = create_destination(dest_cfg["type"], json.loads(dest_cfg["config_json"]))
            ...
            if str(local_archive).endswith(".enc"):
                passphrase = self._get_encryption_passphrase()
                if not passphrase:
                    return False, "Encrypted backup requires BACKUP_ENCRYPTION_PASSPHRASE"
            ...
            for file_entry in manifest.get("contents", []):
                ...
                if os.path.isfile(fp):
                    ...
                    if actual and file_entry.get("checksum_sha256") and actual != file_entry.get("checksum_sha256"):
                        raise ValueError(f"Checksum mismatch for {rel}")

            # Step 3: Safety backup of current state (best-effort)
            try:
                emergency_dest_id = destination_id or self._pick_fallback_destination_id()
            ...
            db_map = {
                "devices": (os.path.join(restore_root, "devices.db"), self.config.get_device_store_config()["database"]),
                "users": (os.path.join(restore_root, "local_auth.db"), self.config.get_local_auth_db()),
                "accounting": (
                ...
                "overrides": (os.path.join(restore_root, "config_overrides.db"), "data/config_overrides.db"),
            }
    ...
    def create_manual_backup(self, destination_id: str, created_by: str) -> str:
        ...
        return self.execute_backup(destination_id, triggered_by=created_by, backup_type="manual")
