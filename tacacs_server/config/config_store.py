...
class ConfigStore:
    ...
    def clear_overrides(
        self, section: str | None = None, changed_by: str = "system"
    ) -> None:
        """
    ...
    def _add_history(
        ...
        old_val_str = (
            None if old_value is None else self._encode_value(old_value, value_type)
        )
        new_val_str = (
            None if new_value is None else self._encode_value(new_value, value_type)
        )

        # Compute config hash if full config provided
        config_hash = (
            compute_config_hash(json.dumps(full_config)) if full_config else ""
        )
