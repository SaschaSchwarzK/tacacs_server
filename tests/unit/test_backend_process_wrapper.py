...
def test_backend_times_out_and_returns_timed_out_flag():
    ...
    handler = AAAHandlers(auth_backends=[SlowBackend("slow", sleep_s=1.0)], db_logger=None, backend_timeout=0.25)
    backend = handler.auth_backends[0]

    ok, timed_out, err = handler._authenticate_backend_with_timeout(backend, "u", "p", timeout_s=handler.backend_timeout)
