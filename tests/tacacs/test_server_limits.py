...
def test_packet_length_cap_integration(tacacs_server):
    host = tacacs_server["host"]
    port = tacacs_server["port"]
    # Build a header advertising too large length (e.g., 10k)
    hdr = _mk_header(
        TAC_PLUS_MAJOR_VER, TAC_PLUS_PACKET_TYPE.TAC_PLUS_AUTHEN, 1, 0, 0x9999, 10000
    )
    ...
    try:
        s.sendall(hdr)
        # Server should drop connection; a recv should return b'' or timeout
        try:
            data = s.recv(1)
        except TimeoutError:
            data = b""
