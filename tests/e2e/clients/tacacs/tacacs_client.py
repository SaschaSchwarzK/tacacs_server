...
def main() -> int:
    ...
    ap.add_argument(
        "--allow-reject",
        action="store_true",
        help="Exit 0 when server responds even if authentication is rejected",
    )
    args = ap.parse_args()
    ...
    if args.allow_reject and (
        msg.startswith("authentication rejected")
        or msg.startswith("status=")
        or msg.startswith("authentication accepted")
        or msg.startswith("Connection error") is False
    ):
        return 0
    return 1
