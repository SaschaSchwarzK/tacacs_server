...
def test_tacacs_via_two_proxies(tmp_path: Path) -> None:
    ...
    try:
        ...
        _run([
            ...
            "-p",f"{tacacs_port}:5049","-p",f"{api_port}:8080",
            "-e",f"API_TOKEN={api_token}",
            "-v",f"{tmp_cfg}:/app/config/tacacs.container.ini:ro",
            server_image,
            "sh","-lc","/opt/venv/bin/tacacs-server --config /app/config/tacacs.container.ini"
        ])
        ...
        # Users and groups
        _api_post(sess, f"{base_url}/api/users", {"username":"user_accept","password":"GoodPass1!","privilege_level":15,"service":"exec","groups":[],"enabled":True}, tolerate_exists=True)
        _api_post(sess, f"{base_url}/api/users", {"username":"user_reject","password":"BadPass1!","privilege_level":1,"service":"exec","groups":[],"enabled":True}, tolerate_exists=True)
        # Group for real clients (correct secret)
        _api_post(sess, f"{base_url}/api/device-groups", {"name":"e2e-clients","description":"Clients","tacacs_secret":"TacacsSecret123!"}, tolerate_exists=True)
        # Group for proxies (wrong secret) — if PROXY v2 is not honored, requests will match these /32 entries and fail
        _api_post(sess, f"{base_url}/api/device-groups", {"name":"proxy-devices","description":"Proxies","tacacs_secret":"WrongSecret456!"}, tolerate_exists=True)
        dg = sess.get(f"{base_url}/api/device-groups", timeout=10).json()
        gids = {str(i.get("name","" )).lower(): int(i.get("id",0)) for i in dg if isinstance(i,dict)}
        ...
        _api_post(sess, f"{base_url}/api/devices", {"name":"any","ip_address":"0.0.0.0/0","device_group_id":gid_clients,"enabled":True}, tolerate_exists=True)

        # Proxies
        proxy1 = f"proxy1-{unique}"
        proxy2 = f"proxy2-{unique}"
        _run(["docker","run","-d","--name",proxy1,"--network",network_name,"-e",f"BACKEND_HOST={server_container}", proxy_image])
        _run(["docker","run","-d","--name",proxy2,"--network",network_name,"-e",f"BACKEND_HOST={server_container}", proxy_image])
        ...
        p1 = subprocess.run([
            "docker","run","--name",client1,"--network",network_name,"--entrypoint","",unified_image,
            "sh","-lc",
            f"ip=$(hostname -i | awk '{{print $1}}'); echo CLIENT_IP=$ip; python /app/client.py --mode tacacs --host {proxy1} --port 5049 --secret TacacsSecret123! --username user_accept --password GoodPass1!"
        ], check=False, capture_output=True, text=True)
        out1 = (p1.stdout or "") + ("\n" + p1.stderr if p1.stderr else "")
        if p1.returncode != 0:
            raise AssertionError(f"tacacs_proxy1_accept failed (rc={p1.returncode}).\n--- client output ---\n{out1}\n--- server logs ---\n" + _logs(server_container, 1000))
        c1_ip = ""
        for line in out1.splitlines():
            if line.startswith("CLIENT_IP="):
                c1_ip = line.split("=",1)[1].strip()
                break
        ...
        if proxy1_ip:
            _api_post(sess, f"{base_url}/api/devices", {"name":"proxy1","ip_address":f"{proxy1_ip}/32","device_group_id":gid_proxies,"enabled":True}, tolerate_exists=True)
        subprocess.run(["docker","rm","-f",client1], check=False)
        _assert_cmd(["docker","run","--rm","--network",network_name,"--entrypoint","",unified_image,
                     "sh","-lc",f"python /app/client.py --mode tacacs --host {proxy1} --port 5049 --secret TacacsSecret123! --username user_reject --password WrongPass!"],
                    server_container, label="tacacs_proxy1_reject", expect_zero=False)

        # TACACS via proxy2
        time.sleep(0.5)
        client2 = f"client2-{unique}"
        p2 = subprocess.run([
            "docker","run","--name",client2,"--network",network_name,"--entrypoint","",unified_image,
            "sh","-lc",
            f"ip=$(hostname -i | awk '{{print $1}}'); echo CLIENT_IP=$ip; python /app/client.py --mode tacacs --host {proxy2} --port 5049 --secret TacacsSecret123! --username user_accept --password GoodPass1!"
        ], check=False, capture_output=True, text=True)
        out2 = (p2.stdout or "") + ("\n" + p2.stderr if p2.stderr else "")
        if p2.returncode != 0:
            raise AssertionError(f"tacacs_proxy2_accept failed (rc={p2.returncode}).\n--- client output ---\n{out2}\n--- server logs ---\n" + _logs(server_container, 1000))
        c2_ip = ""
        for line in out2.splitlines():
            if line.startswith("CLIENT_IP="):
                c2_ip = line.split("=",1)[1].strip()
                break
        # Register proxy2 IP as a /32 device with wrong secret
        proxy2_ip = _ip(proxy2)
        if proxy2_ip:
            _api_post(sess, f"{base_url}/api/devices", {"name":"proxy2","ip_address":f"{proxy2_ip}/32","device_group_id":gid_proxies,"enabled":True}, tolerate_exists=True)
        subprocess.run(["docker","rm","-f",client2], check=False)
        _assert_cmd(["docker","run","--rm","--network",network_name,"--entrypoint","",unified_image,
                     "sh","-lc",f"python /app/client.py --mode tacacs --host {proxy2} --port 5049 --secret TacacsSecret123! --username user_reject --password WrongPass!"],
                    server_container, label="tacacs_proxy2_reject", expect_zero=False)
...
def _ip(name: str) -> str:
    p = subprocess.run(["docker","inspect","-f","{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}", name], check=True, capture_output=True, text=True)
...
def _assert_cmd(cmd: list[str], server_container: str, label: str, expect_zero: bool) -> None:
    ...
    if (expect_zero and rc != 0) or ((not expect_zero) and rc == 0):
        logs = subprocess.run(["docker","logs","--tail","1000",server_container], check=False, capture_output=True, text=True)
        server_logs = (logs.stdout or "") + ("\n" + logs.stderr if logs.stderr else "")
        raise AssertionError(f"{label} failed (rc={rc}).\n--- client output ---\n{out}\n--- server logs ---\n{server_logs}")
...
def _exec(name: str, cmd: str) -> str:
    try:
        p = subprocess.run(["docker","exec",name,"sh","-lc",cmd], check=False, capture_output=True, text=True)
        return (p.stdout or "") + ("\n" + p.stderr if p.stderr else "")
    except Exception:
        return "(exec failed)"
...
def _logs(name: str, tail: int = 1000) -> str:
    try:
        p = subprocess.run(["docker","logs","--tail", str(tail), name], check=False, capture_output=True, text=True)
        return (p.stdout or "") + ("\n" + p.stderr if p.stderr else "")
    except Exception:
