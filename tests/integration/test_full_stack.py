...
def test_full_stack_all_components(server_factory):
    ...
    with server:
        ...
        device_store.ensure_device(
            name="stack-device", network="127.0.0.1", group="default"
        )
        time.sleep(0.5)
        tacacs_success = tacacs_authenticate(
            "127.0.0.1", server.tacacs_port, "stacksecret", "stackuser", "StackPass123"
        )
...
def test_tacacs_and_admin_api_interaction(server_factory):
    ...
    with server:
        ...
        response = session.post(
            ...
            json={"username": "user2", "password": "Passw0rd2", "privilege_level": 10},
            timeout=5,
        )
...
def test_multiple_server_instances_isolation(server_factory, tmp_path):
    ...
    server1 = ServerInstance(
        work_dir=work_dir1,
        config={"admin": {"username": "admin1", "password": "Passw0rd1"}},
        enable_tacacs=True,
    )
    server2 = ServerInstance(
        work_dir=work_dir2,
        config={"admin": {"username": "admin2", "password": "Passw0rd2"}},
        enable_tacacs=True,
    )
    with server1:
        with server2:
...
def test_component_enable_disable_combinations(server_factory):
    ...
    with server:
        ...
        assert server.web_port is None
    server = server_factory(
        config={"admin": {"username": "admin", "password": "admin123"}},
        enable_tacacs=False,
        enable_radius=False,
        enable_admin_api=True,
        enable_admin_web=False,
...
def test_logs_contain_all_activity(server_factory):
    ...
    with server:
        ...
        device_store.ensure_device(
            name="log-device", network="127.0.0.1", group="default"
        )
        time.sleep(0.5)
        tacacs_authenticate(
