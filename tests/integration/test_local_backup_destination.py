...
def test_local_destination_lifecycle(tmp_path: Path):
    ...
    _orig_allowed = list(_pp.ALLOWED_ROOTS)
    _orig_default = _pp.DEFAULT_BACKUP_ROOT

    if forced_root not in _pp.ALLOWED_ROOTS:
        _pp.ALLOWED_ROOTS.append(forced_root)
    _pp.DEFAULT_BACKUP_ROOT = forced_root
    ...
    try:
        ...
        # Restore original validator and policy settings
