...
def _mk_acct_body(username: str, flags: int, args: list[str]) -> bytes:
    ...
    head = struct.pack("!BBBBBBBBB", flags, 0, 1, 1, 1, len(user_b), len(port_b), len(rem_b), len(arg_bytes))
    body = head
    for ab in arg_bytes:
...
def _send_acct(host: str, port: int, session_id: int, body: bytes) -> int | None:
    pkt = TacacsPacket(version=(TAC_PLUS_MAJOR_VER << 4) | 0, packet_type=TAC_PLUS_PACKET_TYPE.TAC_PLUS_ACCT, seq_no=1, flags=TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG, session_id=session_id, length=0, body=body)
...
def _db_rows(db_path: str, where: str = "", params: tuple = ()) -> list[tuple]:
    ...
    try:
        cur = con.cursor()
        q = "SELECT username, session_id, status, service, command FROM accounting_logs " + where
...
def _wait_for_table(db_path: str, timeout: float = 5.0) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            try:
                cur = con.cursor()
                cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='accounting_logs'")
                row = cur.fetchone()
                if row:
                    return
            finally:
...
def test_accounting_start_update_stop(server_factory):
    ...
    server = server_factory(enable_tacacs=True, config={"database": {"accounting_db": acc_db}})
    with server:
        host, port = "127.0.0.1", server.tacacs_port
        session = int(time.time()) & 0xFFFFFFFF
        _wait_for_table(_db_path(server))
        b_start = _mk_acct_body("acctuser", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=shell", "cmd=session-start", "bytes_in=0", "bytes_out=0"])
        st = _send_acct(host, port, session, b_start)
        assert st in (TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_SUCCESS, TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_ERROR, 0, None)
        b_upd = _mk_acct_body("acctuser", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_WATCHDOG, ["service=shell", "cmd=progress", "bytes_in=100", "bytes_out=64"])
        su = _send_acct(host, port, session, b_upd)
        assert su in (TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_SUCCESS, TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_ERROR, 0, None)
        b_stop = _mk_acct_body("acctuser", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_STOP, ["service=shell", "cmd=session-stop", "bytes_in=200", "bytes_out=120"])
        ss = _send_acct(host, port, session, b_stop)
        assert ss in (TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_SUCCESS, TAC_PLUS_ACCT_STATUS.TAC_PLUS_ACCT_STATUS_ERROR, 0, None)
        time.sleep(0.5)
...
def test_accounting_flags_variations(server_factory):
    ...
    with server:
        ...
        _wait_for_table(_db_path(server))
        for flag, expect in [(TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, "START"), (TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_WATCHDOG, "UPDATE"), (TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_STOP, "STOP")]:
            body = _mk_acct_body("userf", flag, ["service=shell", f"cmd={expect.lower()}"])
...
def test_accounting_session_correlation(server_factory):
    ...
    with server:
        ...
        _wait_for_table(_db_path(server))
        for sess in (s1, s2):
            _send_acct(host, port, sess, _mk_acct_body("alpha", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=shell"]))
            _send_acct(host, port, sess, _mk_acct_body("alpha", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_STOP, ["service=shell"]))
        time.sleep(0.3)
...
def test_accounting_db_storage_verification(server_factory):
    ...
    with server:
        ...
        _wait_for_table(_db_path(server))
        _send_acct(host, port, sess, _mk_acct_body("delta", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=shell", "cmd=run", "bytes_in=10", "bytes_out=5", "elapsed_time=1"]))
        time.sleep(0.3)
...
def test_accounting_concurrent_sessions(server_factory):
    ...
    with server:
        ...
        _wait_for_table(_db_path(server))
        for sid in sessions:
            _send_acct(host, port, sid, _mk_acct_body("gamma", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=exec"]))
        time.sleep(0.5)
...
def test_accounting_disconnect_mid_session(server_factory):
    ...
    with server:
        ...
        _send_acct(host, port, pre_sid, _mk_acct_body("init", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=shell"]))
        _wait_for_table(_db_path(server))
        sid = (int(time.time()) + 50) & 0xFFFFFFFF
        body = _mk_acct_body("omega", TAC_PLUS_ACCT_FLAG.TAC_PLUS_ACCT_FLAG_START, ["service=shell"])
        pkt = TacacsPacket(version=(TAC_PLUS_MAJOR_VER << 4) | 0, packet_type=TAC_PLUS_PACKET_TYPE.TAC_PLUS_ACCT, seq_no=1, flags=TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG, session_id=sid, length=0, body=body)
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(2)
        s.connect((host, port))
        s.sendall(pkt.pack(""))
