...
def _mk_acct_body(username: str, flags: int, args: list[str]) -> bytes:
    """Build minimal TACACS+ accounting request body.

    Order expected by parser: header -> arg_lens[] -> user -> port -> rem -> args
    """
...
def _db_path_from_logs(server) -> str | None:
    ...
    if not candidates:
        return None
    # Prefer a test-specific DB if present
    for c in reversed(candidates):
        if "test_accounting_" in c:
            return c
    # Otherwise use the last candidate
    return candidates[-1]
    return None
...
def _wait_for_table(db_path: str, timeout: float = 5.0) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.05)
    # If we get here, table wasn't created in time
    raise RuntimeError(f"accounting_logs not found in DB within {timeout}s: {db_path}")


def _dump_server_logs(server, note: str = "") -> None:
    try:
        logs = server.get_logs()
        print(f"\n[ACCOUNTING-DEBUG] {note} -- DB path guess: {_db_path(server)}")
        print("[ACCOUNTING-DEBUG] --- server log tail ---")
        print(logs[-4000:])
        print("[ACCOUNTING-DEBUG] --- end log tail ---\n")
    except Exception:
        pass
...
def test_accounting_start_update_stop(server_factory):
    ...
    with server:
        ...
        dbp = _db_path(server)
        try:
            rows = _db_rows(dbp, "WHERE session_id=?", (session,))
        except Exception as e:
            _dump_server_logs(server, f"query session_id={session} failed: {e}")
            raise
...
def test_accounting_flags_variations(server_factory):
    ...
    with server:
        ...
        try:
            ...
            _dump_server_logs(server, f"flags variations: {e}")
            raise
...
def test_accounting_session_correlation(server_factory):
    ...
    with server:
        ...
        try:
            ...
            _dump_server_logs(server, f"session correlation: {e}")
            raise
...
def test_accounting_db_storage_verification(server_factory):
    ...
    with server:
        ...
        try:
            ...
            _dump_server_logs(server, f"storage verification: {e}")
            raise
        assert rows, "No accounting rows stored"
        # Verify username, session_id, status
...
def test_accounting_concurrent_sessions(server_factory):
    ...
    with server:
        ...
        try:
            ...
            _dump_server_logs(server, f"concurrent sessions: {e}")
            raise
...
def test_accounting_disconnect_mid_session(server_factory):
    ...
    with server:
        host, port = "127.0.0.1", server.tacacs_port
        # Pre-flight: trigger DB initialization by sending a normal START
        ...
        # Wait for schema now that the logger has been touched
        _wait_for_table(_db_path(server))
        # Now perform the disconnect mid-session scenario
        ...
        s.connect((host, port))
        s.sendall(pkt.pack(""))
        # Intentionally do not read response; close socket to simulate disconnect
        s.close()
        # Poll up to a few seconds for async DB write to appear
        rows = []
        ...
        while time.time() < deadline:
            try:
                ...
                _dump_server_logs(server, f"disconnect mid-session query error: {e}")
                raise
            if any(r[2] == "START" for r in rows):
                break
            time.sleep(0.1)
        if not any(r[2] == "START" for r in rows):
            _dump_server_logs(server, "disconnect mid-session: START not found")
