...
def test_config_api_full_workflow(server_factory):
    ...
    with server:
        ...
        assert response.json().get("valid") is False, "Expected validation to fail for unknown backend"
        ...
        response = session.put(f"{base}/api/admin/config/auth", json=bad_update, timeout=5)
        assert response.status_code == 400, f"Expected 400 for invalid update, got {response.status_code}"
        ...
        assert response.status_code == 404, f"Expected 404 for non-existent section, got {response.status_code}"
        ...
        assert isinstance(versions, list), f"Expected versions list, got {type(versions)}"

        # Test history
        response = session.get(
            f"{base}/api/admin/config/history", 
            params={"limit": 5}, 
            timeout=5
        )
        ...
        assert response.status_code == 200, f"Failed to update server config: {response.text}"
        ...
        assert any(h.get("section") == "server" for h in hist), "Expected history to include recent change"
        ...
        assert len(versions) >= count0 + 1, "Expected at least one new version after update"
        ...
        if versions:
            ...
            rr = session.post(f"{base}/api/admin/config/versions/{latest_ver}/restore", timeout=5)
