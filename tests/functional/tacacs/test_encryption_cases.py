...
def _mk_header(version_major: int, ptype: int, seq: int, flags: int, session: int, length: int) -> bytes:
...
def _mk_auth_body(username: str, password: str) -> bytes:
    ...
    return (
        struct.pack("!BBBBBBBB", 1, 15, 2, 1, len(user_b), len(port_b), len(rem_b), len(data_b))
...
def _setup_device_and_user(server, username: str, password: str, secret: str) -> None:
    ...
    store.ensure_group("default", description="Default group", metadata={"tacacs_secret": secret})
    store.ensure_device(name="test-device", network="127.0.0.1", group="default")

def _auth_once(host: str, port: int, secret: str, username: str, password: str, *, session_id: int, seq: int, flags: int = 0) -> tuple[bool, str]:
    ...
    try:
        ...
        enc = body if (flags & TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG) else _xor_body(body, session_id, secret, version, seq)
        hdr = _mk_header(TAC_PLUS_MAJOR_VER, TAC_PLUS_PACKET_TYPE.TAC_PLUS_AUTHEN, seq, int(flags), session_id, len(enc))
        s.sendall(hdr + enc)
        rh = s.recv(12)
        if len(rh) != 12:
        ...
        resp = rb if (rflags & TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG) else _xor_body(rb, rsess, secret, rver, rseq)
        if len(resp) < 6:
            return False, "short"
...
def _auth_on_socket(sock: socket.socket, secret: str, username: str, password: str, *, session_id: int, seq: int, flags: int = 0) -> tuple[bool, int]:
    version = ((TAC_PLUS_MAJOR_VER & 0x0F) << 4) | 0
    body = _mk_auth_body(username, password)
    enc = body if (flags & TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG) else _xor_body(body, session_id, secret, version, seq)
    hdr = _mk_header(TAC_PLUS_MAJOR_VER, TAC_PLUS_PACKET_TYPE.TAC_PLUS_AUTHEN, seq, int(flags), session_id, len(enc))
...
def test_encryption_with_special_characters_secret(server_factory):
    ...
    with server:
        ...
        ok, msg = _auth_once("127.0.0.1", server.tacacs_port, secret, "specuser", "SpecPass1", session_id=(int(time.time()) & 0xFFFFFFFF), seq=1)
...
def test_encryption_with_very_long_secret(server_factory):
    ...
    with server:
        ...
        ok, msg = _auth_once("127.0.0.1", server.tacacs_port, secret, "longuser", "LongPass1", session_id=(int(time.time()) & 0xFFFFFFFF), seq=1)
...
def test_encryption_with_non_ascii_secret(server_factory):
    ...
    with server:
        ...
        ok, msg = _auth_once("127.0.0.1", server.tacacs_port, secret, "uniuser", "UniPass1", session_id=(int(time.time()) & 0xFFFFFFFF), seq=1)
        assert ok, f"Auth should succeed with non-ASCII secret: {msg}"

@pytest.mark.integration
def test_mixed_encrypted_unencrypted_sessions(server_factory):
    server = server_factory(enable_tacacs=True, config={"security": {"encryption_required": "false"}})
    with server:
        secret = "allow-plain"
        _setup_device_and_user(server, "plainuser", "PlainPass1", secret)
        sess1 = int(time.time()) & 0xFFFFFFFF
        sess2 = (int(time.time()) + 1) & 0xFFFFFFFF
        ok1, _ = _auth_once("127.0.0.1", server.tacacs_port, secret, "plainuser", "PlainPass1", session_id=sess1, seq=1)
        ok2, _ = _auth_once("127.0.0.1", server.tacacs_port, secret, "plainuser", "PlainPass1", session_id=sess2, seq=1, flags=TAC_PLUS_FLAGS.TAC_PLUS_UNENCRYPTED_FLAG)
        assert ok1 and ok2, "Both encrypted and unencrypted auth should succeed when not required"
        time.sleep(0.1)
...
def test_encryption_key_rotation_session_cache(server_factory):
    ...
    with server:
        ...
        try:
            ok1, _ = _auth_on_socket(sock, secret1, "rotuser", "RotPass1", session_id=sess, seq=1)
            assert ok1, "First auth with S1 should pass"
            from tacacs_server.devices.store import DeviceStore
            store = DeviceStore(str(server.devices_db))
            store.ensure_group("default", description="Default group", metadata={"tacacs_secret": "rotate-TWO-KEY"})
            secret2 = "rotate-TWO-KEY"
            ok2, _ = _auth_on_socket(sock, secret1, "rotuser", "RotPass1", session_id=sess, seq=3)
            if ok2:
                ok3, _ = _auth_on_socket(sock, secret2, "rotuser", "RotPass1", session_id=sess, seq=5)
                assert not ok3, "Expected new secret rejected if old still cached for session"
            else:
                ok3, _ = _auth_on_socket(sock, secret2, "rotuser", "RotPass1", session_id=sess, seq=5)
                assert not ok3, "Expected new secret also rejected mid-session when old was rejected"
                ok_new, _ = _auth_once(host, port, secret2, "rotuser", "RotPass1", session_id=((sess + 12345) & 0xFFFFFFFF), seq=1)
                assert ok_new, "Expected new secret to succeed on a fresh session after rotation"
        finally:
            try:
                sock.close()
            except Exception:
