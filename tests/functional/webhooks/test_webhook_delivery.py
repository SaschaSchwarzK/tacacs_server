    ...
    try:
        with server:
            ...
            try:
                ...
                sess.get(f"{base}/api/admin/webhooks-config", timeout=5)
                sess.put(
            ...
            event_field = (
                "event_type" if "event_type" in records1[0]["payload"] else "event"
            )
            assert records1[0]["payload"][event_field] in [
                "auth_failure",
                "threshold_exceeded",
            ]
            assert records2[0]["payload"][event_field] in [
                "auth_failure",
                "threshold_exceeded",
            ]
    finally:
